...
public class FeedbackActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		solutionCodeTextView=(TextView) this.findViewById(R.id.chosed_solution);
		...
		final String[] windsStrings = new String[winds.size()];
		for (int i = 0; i < windsStrings.length; i++) {
			windsStrings[i] = winds.get(i).getName();
		}
		...
		sysSolutionView.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				// TODO dialog / new thread:check if the tree exists
				// if exists,check version.if not exist,download and show.
